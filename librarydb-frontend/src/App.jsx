...
function NavItem({ icon, label, active = false, onClick }) {
  return (
    ...
    >
      ...
      {active && (
        <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-[color:var(--accent)] rounded-r-md" />
      )}
    </button>
  );
}
  ...
  return (
    <div className="group flex flex-col">
      ...
      <div className="pr-2">
        ...
        <p className="text-sm text-[color:var(--text-secondary)] line-clamp-1">
          {book.author}
        </p>
        {book.quantity === 0 && (
...
// Tailwind needs to see all possible grid classes
...
export default function App() {
  ...
  // Load books (currently from fakeData via api/books.js)
  useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    return books
      ...
      .filter((b) => (filters.onlyInStock ? b.quantity > 0 : true))
      .filter((b) => (b.condition ?? 1) >= filters.minCondition)
      .filter((b) => (filters.author ? b.author === filters.author : true))
      .filter((b) =>
        filters.publisher ? b.publisher === filters.publisher : true
      )
      .filter((b) => {
  ...
  }, [books, searchTerm, activeLetter, filters]);
        ...
        <aside className="w-64 bg-[color:var(--sidebar-bg)] border-r border-[color:var(--border)] flex flex-col fixed h-full z-20">
          ...
          <nav className="flex-1 px-4 space-y-8 overflow-y-auto py-4">
            <div className="space-y-1">
              ...
              <NavItem
                icon={<Plus size={20} />}
                label="Add Items"
                onClick={() => alert("Placeholder: Add Items")}
              />
              <NavItem
              ...
              <NavItem
                icon={<Upload size={20} />}
                label="Publish"
                onClick={() => alert("Placeholder: Publish")}
              />
            </div>
          ...
          <div className="p-4 space-y-1 bg-[color:var(--sidebar-bg)] border-t border-[color:var(--border)]">
            ...
            <NavItem
              icon={<LogOut size={20} />}
              label="Logout"
              onClick={() => alert("Placeholder: Logout")}
            />
          </div>
        </aside>

        {/* Main (capped & centered) */}
        <main className="ml-64 flex-1 px-12 py-8 w-full">
            ...
            {activePage === "library" && (
              <>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
                  <div className="flex items-center rounded-lg p-1.5 pr-4 bg-[color:var(--active-bg)]">
                    ...
                    <div className="ml-2">
                      <ArrowDown
                        size={16}
                        className="opacity-60 rotate-[-90deg]"
                      />
                    </div>
                  </div>
                ...
                <h2 className="text-2xl font-bold mb-8 text-[color:var(--accent)]">
                  {activeLetter === "ALL" ? "A" : activeLetter}
                </h2>
